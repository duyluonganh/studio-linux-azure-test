...
public class DriverFactory2 {
    ...
    public static final String REMOTE_DRIVER_PROPERTY = StringConstants.CONF_PROPERTY_REMOTE_DRIVER;
...
//            EventBusSingleton.getInstance().getEventBus().post(browserOpenedEvent);
    ...
    private static DesiredCapabilities addKatalonExtensionsToChrome(DesiredCapabilities capabilities) {
        boolean isInstalledKatalonCompactUtility = WebDriverPropertyUtil
                .didUserProfileInstallKatalonCompactUtility(capabilities);
...
//    }
    ...
    private static Factory getClientFactoryForRemoteDriverExecutor(ProxyInformation proxyInfo, URL url)
        ...
        String rawTimeout = RunConfiguration.getDriverSystemProperty(RunConfiguration.REMOTE_DRIVER_PROPERTY,
                "readTimeout");
        int readTimeout = rawTimeout != null ? Integer.valueOf(rawTimeout) * 60 : DEFAULT_READ_TIMEOUT_IN_SECONDS;
...
//    }
    ...
    private static DesiredCapabilities addSmartWaitExtensionToEdgeChromium(DesiredCapabilities desiredCapabilities) {
        if (shouldInstallSmartWait()) {
            String extensions = "";
            String pathToSmartWaitExtension = installExtensionFromFile(getEdgeChromiumExtensionFile());
            extensions = LOAD_EXTENSION_EDGE_CHROMIUM_PREFIX + pathToSmartWaitExtension;
    ...
    private static void logBrowserRunData(WebDriver webDriver) {
        ...
        if (StringUtils.isNotEmpty(serverUrl) && webDriver instanceof RemoteWebDriver) {
            String testCloudRemoteOS = RunConfiguration
                    .getDriverSystemProperty(RunConfiguration.REMOTE_DRIVER_PROPERTY, "testCloudRemoteOS");
            ...
            switch (webUIDriver) {
                ...
                case EDGE_CHROMIUM_DRIVER:
                    ...
                    if (options instanceof DesiredCapabilities) {
                        ...
                        return new CEdgeDriver(edgeService, (DesiredCapabilities) options, getActionDelay());
